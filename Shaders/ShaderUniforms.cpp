...
bool ShaderUniform::Map(IShaderProgram* program) {
    ...
    if(MR::MachineInfo::CatchError(0, &gl_er)) {
        std::string err_str = "Error in ShaderUniform::Map : glGetUniformLocation("+std::to_string(program->GetGPUHandle())+", \""+_name+"\") ended with \"" + std::to_string(gl_er) + "\" code. ";
        switch(gl_er) {
    ...
    if(_gpu_location == -1) {
#ifdef MR_CHECK_SMALL_GL_ERRORS
        MR::Log::LogString("Error in ShaderUniform::Map : glGetUniformLocation("+std::to_string(program->GetGPUHandle())+", \""+_name+"\") == -1.", MR_LOG_LEVEL_ERROR);
#endif
        return false;
    }
