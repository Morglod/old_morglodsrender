...
void MR::Camera::AttachToShader(IShaderProgram* shader) {
    ...
    if(shader){
        ...
        if(su) {
            as._mvpUniform = su;
            as._mvpUniform->SetPtr(&(*_mvp)[0][0]);
        }
        else as._mvpUniform = shader->CreateUniform(MR_SHADER_MVP_MAT4, IShaderUniform::SUT_Mat4, &(*_mvp)[0][0]);

        su = shader->FindShaderUniform(MR_SHADER_MODEL_MAT4);
        if(su) {
            as._modelUniform = su;
            as._modelUniform->SetPtr(&(*_modelMatrix)[0][0]);
        }
        else as._modelUniform = shader->CreateUniform(MR_SHADER_MODEL_MAT4, IShaderUniform::SUT_Mat4, &(*_modelMatrix)[0][0]);

        su = shader->FindShaderUniform(MR_SHADER_VIEW_MAT4);
        if(su) {
            as._viewUniform = su;
            as._viewUniform->SetPtr(&(*_viewMatrix)[0][0]);
        }
        else as._viewUniform = shader->CreateUniform(MR_SHADER_VIEW_MAT4, IShaderUniform::SUT_Mat4, &(*_viewMatrix)[0][0]);

        su = shader->FindShaderUniform(MR_SHADER_PROJ_MAT4);
        if(su) {
            as._projUniform = su;
            as._projUniform->SetPtr(&(*_projectionMatrix)[0][0]);
        }
        else as._projUniform = shader->CreateUniform(MR_SHADER_PROJ_MAT4, IShaderUniform::SUT_Mat4, &(*_projectionMatrix)[0][0]);

        su = shader->FindShaderUniform(MR_SHADER_CAM_POS);
        if(su) {
            as._posUniform = su;
            as._posUniform->SetPtr(&(*_pos));
        }
        else as._posUniform = shader->CreateUniform(MR_SHADER_CAM_POS, MR::IShaderUniform::SUT_Vec3, &(*_pos));

        su = shader->FindShaderUniform(MR_SHADER_CAM_DIR);
        if(su) {
            as._dirUniform = su;
            as._dirUniform->SetPtr(&_forward_direction);
        }
        else as._dirUniform = shader->CreateUniform(MR_SHADER_CAM_DIR, MR::IShaderUniform::SUT_Vec3, &_forward_direction);
...
void MR::Camera::UpdateShader(IShaderProgram* shader) {
    shader->SetUniform(MR_SHADER_MVP_MAT4, *_mvp);
    shader->SetUniform(MR_SHADER_MODEL_MAT4, *_modelMatrix);
    shader->SetUniform(MR_SHADER_VIEW_MAT4, *_viewMatrix);
    shader->SetUniform(MR_SHADER_PROJ_MAT4, *_projectionMatrix);
    shader->SetUniform(MR_SHADER_CAM_POS, *_pos);
    shader->SetUniform(MR_SHADER_CAM_DIR, _forward_direction);
}

void MR::Camera::MoveForward(const glm::vec3& v) {
    if(v == glm::vec3(0,0,0)) return;
    *_pos += v * _forward_direction;
...
void MR::Camera::SetDirection(const glm::vec3& d, const glm::vec3& left_d, const glm::vec3& up_d) {
    if( _forward_direction != d ) {
        _forward_direction = d;
...
void MR::Camera::CalcViewMatrix() {
    ...
    if(_cmode == CameraMode::Direction) *_viewMatrix = glm::lookAt(*_pos, *_pos + _forward_direction, _up_direction);
}
...
MR::Camera::Camera(const glm::vec3& camPos, const glm::vec3& camTarget, const float& fov, const float& nearZ, const float& farZ, const float& aspectR)
    : _cmode(CameraMode::Target), _cproj(CameraProjection::Perspective), _pos( new glm::vec3(camPos) ), _forward_direction(MR::Transform::WorldForwardVector()), _left_direction(MR::Transform::WorldLeftVector()), _up_direction(MR::Transform::WorldUpVector()), _target(camTarget), _up(glm::vec3(0,1,0)),
...
MR::Camera::Camera(const MR::Camera&)
    : _cmode(CameraMode::Target), _cproj(CameraProjection::Perspective), _pos(), _forward_direction(), _left_direction(), _target(), _up(glm::vec3(0,1,0)),
...
void MR::Camera::CalcDirections(){
    if(_cmode == CameraMode::Direction){
        _forward_direction = MR::Transform::NormalizedDirection(_rot.x, _rot.y, _rot.z);
