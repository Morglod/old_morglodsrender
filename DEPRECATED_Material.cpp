...
bool MR::MaterialPass::Use(){
    if( (_flag.always) || (_parent->GetManager()->GetActivedFlag() == _flag.flag) ){
        ...
        rm->SetShaderProgram(_shader);

        if(_ambient)    rm->SetTexture(_ambient, _ambientUnit);
        if(_diffuse)    rm->SetTexture(_diffuse, _diffuseUnit);
        if(_opacity)    rm->SetTexture(_opacity, _opacityUnit);

        if(_twoSided)   glDisable(GL_CULL_FACE);
        else            glEnable(GL_CULL_FACE);

        rm->SetActivedMaterialPass(this);
...
void MR::MaterialPass::UnUse(){
    MR::RenderManager * rm = MR::RenderManager::GetInstance();
    if(rm->GetActivedMaterialPass() != this) return;
    ...
    rm->SetShaderProgram(nullptr);
}

void MR::MaterialPass::SetShader(IShaderProgram* sh) {
    _shader = sh;
    if(sh != nullptr) {
        _ambient_sampler = _shader->CreateUniform(MR_SHADER_AMBIENT_TEX, IShaderUniform::SUT_Sampler2D, &_ambientUnit);
        _diffuse_sampler = _shader->CreateUniform(MR_SHADER_DIFFUSE_TEX, IShaderUniform::SUT_Sampler2D, &_diffuseUnit);
        _opacity_sampler = _shader->CreateUniform(MR_SHADER_OPACITY_TEX, IShaderUniform::SUT_Sampler2D, &_opacityUnit);
